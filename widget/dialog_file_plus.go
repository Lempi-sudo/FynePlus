...
var _ dialog.Dialog = (*FileDialog)(nil)
	...
	f.open = widget.NewButton(label, func() {
		...
		if f.file.save {
			...
			if !exists {
				...
				dialog.ShowInformation("Cannot overwrite",
					"Files cannot replace a directory,\ncheck the file name and try again", f.file.parent)
				return
			}

			dialog.ShowConfirm("Overwrite?", "Are you sure you want to overwrite the file\n"+name+"?",
				func(ok bool) {
					if !ok {
						return
					}
	...
	newFolderButton := widget.NewButtonWithIcon("", theme.FolderNewIcon(), func() {
		...
		dialog.ShowForm("New Folder", "Create Folder", "Cancel", []*widget.FormItem{
			...
			if createFolderErr != nil {
				...
				dialog.ShowError(errors.New("folder cannot be created"), f.file.parent)
			}
			f.refreshDir(f.dir)
		}, f.file.parent)
	})
